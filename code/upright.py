...
def cost_function(image, lines, theta, phi, gamma, f):
...
def derivative_componets():
    ...
    u = Matrix([u1, u2, u3])
    H1u = H1.dot(u)
    H1u_theta = diff(Matrix(H1u), theta)
    H1u_phi = diff(Matrix(H1u), phi)
    H1u_gamma = diff(Matrix(H1u), gamma)
    H1u_f = diff(Matrix(H1u), f)

    v = Matrix([v1, v2, v3])
    H1v = H1.dot(v)
    H1v_theta = diff(Matrix(H1v), theta)
    H1v_phi = diff(Matrix(H1v), phi)
    H1v_gamma = diff(Matrix(H1v), gamma)
    H1v_f = diff(Matrix(H1v), f)
...
def gradient_descent_optimizer(image, max_iters, learning_rate):
    ...
    for i in range(max_iters):
        ...
        # Compute gradient
        [f, theta, phi, gamma], plot_u, plot_v = gradient(image, lines, init_f, init_theta, init_phi, init_gamma)
        # print('Perspective', plot_u, plot_v)
        print('Theta, Phi, Gamma', theta, phi, gamma)

        init_f = np.float32(init_f - learning_rate * f[0])
        init_theta = np.float32(init_theta - learning_rate * theta[0])
        init_phi = np.float32(init_phi - learning_rate * phi[0])
        init_gamma = np.float32(init_gamma - learning_rate * gamma[0])

        h, w, _ = np.shape(image)

        K = np.array([[init_f, 0, - h / 2],
                      [0, init_f, - w / 2],
                      [0, 0, 1]])
