...
def cost_function(image, lines, theta=0, phi=0, gamma=0, f=1, epsilon=0.1):
    ...
    # H function
    H = np.dot(K, np.dot(R, inv(K))) * h

    H1 = 1 / h * np.dot(K, np.dot(R.T, inv(K)))
...
def gradient(image, lines, f_, theta_, phi_, gamma_):
    ...
    # H function
    H1 = 1 / h_ * np.dot(K, np.dot(R.T , inv(K)))
...
def gradient_descent_optimizer(image, max_iters, learning_rate):
    ...
    # Loss function
    rectification = cost_function(image, lines, init_theta, init_phi, init_gamma, init_f)
    print(rectification)
    ...
    for i in range(max_iters):
        ...
        print('Gradient value ', f, theta, phi, gamma)
        ...
        # H function
        H1 = 1 / h * np.dot(K, np.dot(R, inv(K)))
        print(H1)
